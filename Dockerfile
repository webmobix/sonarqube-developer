...
# install SonarQube
WORKDIR /opt
RUN wget -q -O /opt/sq.zip https://binaries.sonarsource.com/CommercialDistribution/sonarqube-developer/sonarqube-developer-8.1.0.31237.zip
RUN unzip -q sq.zip \ 
  && rm /opt/sq.zip \
  && mv /opt/sonarqube-8.1.0.31237/* /opt/sonarqube/ \
  && rm -r /opt/sonarqube-8.1.0.31237 \
